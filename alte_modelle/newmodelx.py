...
def add_obstacle(obs_x,obs_y, width):
...
def check(pos_x, pos_y, obstacles):
    #while obstacles:
    ...
    for obstacle in obstacles:
        if (pos_x - obstacle[0]) ** 2 + (pos_y - obstacle[1]) ** 2 <= obstacle[2] ** 2:
            return False
    return True
    # True bedeutet dass das boot trefft keine obstacle.

#def p_obstacles(pos_x,pos_y,obstacles,i=0):
#    for obstacle in obstacles:
#        d = (pos_x - obstacle[0])**2+(pos_y - obstacle[1])**2
#        i += 1
#        if d < obstacle(2):
#            return i
#    return -1
...
if __name__ == "__main__":  
    add_obstacle(-10,60,10)
    add_obstacle(40,105,10)
    yacht_simulation(0,0,90,40,20)
#      4.560858173
#      2.03557211759  
